...
This script pulls information from the EBMS Drupal 7 production database and
assembles collections of each of the different kinds of corresponding records
needed for the rewritten Drupal 9 implementation of the EBMS. Each of these
collected sets is serialized to a text file, each line of which contains
the JSON-encoding string for a single record. This means that a JSON parser
cannot process one of these files as a unit, but must be fed each line
separately. This approach serves two purposes. First, it avoids having the
JSON parser run out of memory. In addition, this facilitates the task of
identifying changed or added records between two separate runs of this
script.

It is important that the "exported" directory created by a previous run of
the script be moved to "baseline" before the script is run again, in order
to make it possible for the find-deltas-from-baseline.py script to identify
new and changed records from the previous run. The script has been carefully
designed to produce the same JSON string for each record which has not changed
from one run to another in the source database (even, for example, when those
records contain new primary keys for the target database generated by this
script; see the code below which loads IDs from the packet_article_ids file
if that file exists).

The parent directory must contain a copy of dbpw.json, which stores the
credentials required for connecting to the Drupal 7 database. Since this
information is sensitive, it is not committed to the version control
repository.

Command-line options are available for generating a subset of the exported
records, useful for debugging purposes. Run ./export.py --help for the
available options.
...
class Exporter:
    ...
    def articles(self):
        ...
        offset = 0
        batch_size = 10000
        with open("exported/articles.json", "w", encoding="utf-8") as fp:
            while True:
                self.cursor.execute(
    ...
    def boards(self):
        ...
        with open("exported/boards.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                board = dict(
                    id=row["board_id"],
                    name=row["board_name"],
    ...
    def docs(self):
        ...
        with open("exported/docs.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                doc_id = row["doc_id"]
                doc = dict(
                    id=doc_id,
    ...
    def files(self):
        ...
        with open("exported/files.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                if row["uri"]:
                    values = dict(
                        fid=row["fid"],
    ...
    def groups(self):
        ...
        self.cursor.execute("SELECT * FROM ebms_ad_hoc_group "
                            "ORDER BY group_id")
        with open("exported/ad_hoc_groups.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                group_id = row["group_id"]
                group = dict(
                    id=group_id,
        ...
        with open("exported/subgroups.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                group = dict(
                    id=row["sg_id"],
                    name=row["sg_name"],
    ...
    def imports(self):
        ...
        with open("exported/import_batches.json", "w", encoding="utf-8") as fp:
            offset = 0
            batch_size = 5000
            while True:
                self.cursor.execute(
        ...
        path = "exported/import_requests.json"
    ...
    def journals(self):
        ...
        with open("exported/journals.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                journal_id = row["source_jrnl_id"]
    ...
    def meetings(self):
        ...
        with open("exported/meetings.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                nid = row["nid"]
                meeting = dict(
                    id=nid,
    ...
    def messages(self):
        ...
        with open("exported/messages.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                mid = row["mid"]
                values = dict(
                    id=mid,
    ...
    def packets(self):
        ...
        with open("exported/reviewer_docs.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                doc_id = row["doc_id"]
        ...
        offset = 0
        batch_size = 3000
        with open("exported/reviews.json", "w", encoding="utf-8") as fp:
            while True:
                self.cursor.execute(
        ...
        articles = {}
        path = "exported/packet_articles.json"
        with open(path, "w", encoding="utf-8") as fp:
        ...
        with open("packet_article_ids", "w", encoding="utf-8") as fp:
        ...
        with open("exported/packets.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                packet_id = row["packet_id"]
                values = dict(
                    id=packet_id,
        ...
        reviewer docs
        reviews
        packet_articles (reviews)
        packets (articles, reviewers, summaries)
        """
    def states(self):
        ...
        offset = 0
        batch_size = 10000
        with open("exported/states.json", "w", encoding="utf-8") as fp:
            while True:
                self.cursor.execute(
    ...
    def summaries(self):
        ...
        with open("exported/summary_pages.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                page_id = row["page_id"]
                board_id = row["board_id"]
        ...
        path = "exported/board_summaries.json"
        with open(path, "w", encoding="utf-8") as fp:
    ...
    def tags(self):
        ...
        with open("exported/article_tags.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                article_tag_id = row["article_tag_id"]
                tag = dict(
                    id=article_tag_id,
    ...
    def topics(self):
        ...
        topics = []
        with open("exported/topics.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                topic = dict(
                    id=row["topic_id"],
                    name=row["topic_name"],
    ...
    def travel(self):
        ...
        path = "exported/hotel_requests.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                sid = row["sid"]
                values = dict(
        ...
        process = Popen(["php", "fix-reimbursement-values.php"], **opts)
        ...
        path = "exported/reimbursement_requests.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                sid = row["sid"]
                values = dict(
    ...
    def users(self):
        ...
        with open("exported/users.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                uid = row["uid"]
                if uid > 1:
                    user = dict(
        ...
        with open("exported/authmap.json", "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
                    uid=row["uid"],
                    provider="ebms_core",
    ...
    def __article_tag_vocabulary(self):
        ...
        path = "exported/article_tags_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
                    vid="article_tags",
    ...
    def __board_decision_vocabulary(self):
        ...
        path = "exported/board_decisions_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
                    vid="board_decisions",
    ...
    def __doc_tag_vocabulary(self):
        ...
        path = "exported/doc_tags_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
    ...
    def __import_disposition_vocabulary(self):
        ...
        path = "exported/import_dispositions_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                characters = []
    ...
    def __internal_tag_vocabulary(self):
        ...
        path = "exported/internal_tags_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
                    vid="internal_tags",
    ...
    def __print_job_status_vocabulary(self):
        ...
        path = "exported/print_job_statuses_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
    ...
    def __print_job_type_vocabulary(self):
        ...
        path = "exported/print_job_types_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
                    vid="print_job_types",
    ...
    def __rejection_reason_vocabulary(self):
        ...
        path = "exported/rejection_reasons_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
    ...
    def __relationship_type_vocabulary(self):
        ...
        path = "exported/relationship_types_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
    ...
    def __review_disposition_vocabulary(self):
        ...
        path = "exported/review_dispositions_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
                    vid="dispositions",
    ...
    def __state_vocabulary(self):
        ...
        path = "exported/states_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
    ...
    def __topic_group_vocabulary(self):
        ...
        path = "exported/topic_groups_vocabulary.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                values = dict(
                    vid="topic_groups",
    ...
    def __article_topics(self):
        ...
        rows = self.cursor.fetchall()
        path = "exported/article_topics.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in rows:
                article_id = row["article_id"]
                topic_id = row["topic_id"]
    ...
    def __article_relationships(self):
        ...
        path = "exported/article_relationships.json"
        with open(path, "w", encoding="utf-8") as fp:
            for row in self.cursor.fetchall():
                relationship = dict(
